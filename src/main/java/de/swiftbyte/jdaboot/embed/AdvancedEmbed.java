...
public class AdvancedEmbed {
    ...
    public MessageEmbed build(Instant timestamp) {
        ...
        if (!embed.thumbnailUrl().isEmpty() && !processVar(embed.thumbnailUrl()).isEmpty()) builder.setThumbnail(processVar(embed.thumbnailUrl()));
        if (!embed.imageUrl().isEmpty() && !processVar(embed.imageUrl()).isEmpty()) builder.setImage(processVar(embed.imageUrl()));
